...
class LeadsFilterScreen extends StatefulWidget {
  ...
  final String isFrom;
  const LeadsFilterScreen({super.key,required this.search_key,required this.isFrom});

  @override
  State<LeadsFilterScreen> createState() => _FilterSearchScreenState();
}

class _FilterSearchScreenState extends State<LeadsFilterScreen> {
  ...
  void filtterData() async{
    if(widget.isFrom=='my_leads_property'){
      ...
      await pController.getProjectEnquiry(
        searchKeyword: widget.search_key,
        city: '',
        locality: '',
        max_price:controller.budgetMax1.value!=""?"${controller.budgetMax.value.toString()}00000":
          ...
          children: [
            customIconButton(
              ...
              onTap: () {
                ...
                if(widget.isFrom=="my_leads_property"){
                  pController.getPropertyEnquiry(
                    page: '1',
                    searchKeyword: widget.search_key,
                  );
                }else{
                  pController.getProjectEnquiry(
                    page: '1',
                    searchKeyword: widget.search_key,
                  );
                }
          ...
          child: Obx(() {
            return Column(
              ...
              children: [
                ...
                //           items: controller.budgetOptions.map((option) {
                //             final isEnabled = option['value'] > controller.budgetMin.value;
                //             return DropdownMenuItem<int>(
                //               value: option['value'],
                //               enabled: isEnabled,
                //               child: Text(
                ...
                //               print(selectedOption);
                //               controller.budgetMax1.value = convertLabelToNumberString(selectedOption['label']);
                //               filtterData();
                //             }
                //           },
                ...
                boxH05(),
                widget.isFrom=='my_leads_property'?
                Column(
        ...
        child: Obx(() => Row(
          ...
          children: [
            ...
            Text(
              controller.isClear.value==false?
              widget.isFrom=='my_leads_property'?
              "${pController.getProperyEnquiryList.length} Leads":
              "${pController.getProjectEnquiryList.length} Leads":"0 Lead",style: const TextStyle(
            ...
            SizedBox(
                ...
                child:commonButton(
                  ...
                  onPressed: () {
                    ...
                    final propertyList = pController.getProperyEnquiryList.value
                        .map((item) => item as Map<String, dynamic>)
                        .toList();

                    final result =  pController.filterProperties(propertyList, filters);

                    print("property count before filter : ${pController.getProperyEnquiryList.length}");
