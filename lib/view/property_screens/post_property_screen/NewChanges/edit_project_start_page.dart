...
class edit_project_start_page extends StatefulWidget {
  final int index;
  final String isfrom;
  final data;

  const edit_project_start_page(
      {super.key, required this.isfrom, this.data, required this.index});
  @override
  _edit_project_start_pageState createState() =>
      _edit_project_start_pageState();
}


class _edit_project_start_pageState extends State<edit_project_start_page> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.currentViewIndex.value = 1;
      controller.getPropertyProject(widget.data['_id']??"");
      setDataAndInitialize(widget.data);
      controller.selectedPropertyType.value = widget.data['project_type'] ??'';
      controller.propertytype.value = widget.data['project_type'] ??'';
      updatePropertyFields(widget.data['project_type']??"");


    });
  }
  void updatePropertyFields(String propertyType) {
    print("updatePropertyFields==>${propertyType}");
  ...
  void setDataAndInitialize(Map<String, dynamic> data) {
    ...
    int index = controller.residentalpropertyTypes.indexOf(data['project_type'] ??'');
    if (index != -1) {
      controller.selectedProperty.value = index;
      controller.selectedPropertyType.value = data['project_type'] ??'';
      controller.propertytype.value = data['project_type'] ??'';
    }


    // updatePropertyFields("Residential");
    updatePropertyFields(widget.data['project_type']??"");

    int typeIndex =  controller.residentalpropertyTypes.indexOf(controller.propertytype.value);
  ...
  void initializeController() {
    ...
    List<String> bathroomTypes = ["1", "2", "3", "4", "5", "6", "6+"];
    controller.propertyNameController.text =  widget.data['project_name']??"";
    controller.avgProjectPriceController.text =  widget.data['average_project_price']??"";
    controller.configurationController.text =  widget.data['congfigurations']??"";
    // "address_area":  address_area.value,

    controller.address_area.value =  widget.data['address_area']??"";


    controller.ToiletsConroller.text = widget.data['walls_kitchen_toilet'] ??'';
    controller.ServantConroller.text = widget.data['walls_servant_room'] ??'';
    controller.LivingController.text = widget.data['floor_living_dining'] ??'';
    controller.BedroomController.text = widget.data['floor_bedroom'] ??'';
    controller.BalconyController.text = widget.data['floor_balcony'] ??'';
    controller.DiningController.text = widget.data['walls_living_dining'] ??'';
    controller.ToiletsConroller.text = widget.data['walls_kitchen_toilet'] ??'';
    controller.ServantConroller.text = widget.data['walls_servant_room'] ??'';
    controller.ceilingLivingController.text = widget.data['ceiling'] ??'';
    controller.ceilingServantController.text = widget.data['ceilings_servant_room'] ??'';
    controller.counterKitchenController.text = widget.data['counters_kitchen_toilet'] ??'';
    controller.fittingKitchenController.text = widget.data['fittings_fixtures_kitchen_toilet'] ??'';
    controller.fittingToiletController.text = widget.data['fittings_fixtures_servant_room_toilet'] ??'';
    controller.internalDoorController.text = widget.data['door_window_internal_door'] ??'';
    controller.externalGlazingController.text = widget.data['door_window_external_glazing'] ??'';
    controller.electricalController.text = widget.data['electrical'] ??'';
    controller.backupController.text = widget.data['backup'] ??'';
    controller.securitySystemController.text = widget.data['security_system'] ??'';
    controller.internalDoorController.text = widget.data['internal_door'] ??'';
    controller.ExternalGlazingController.text = widget.data['external_glazings'] ??'';

    controller.flatHouseNoController.text = widget.data['flat_name'] ?? widget.data['flat_name'];
    ...
    controller.aboutController.text =
            widget.data['project_description']??"";
    ...
    controller.flatHouseNoController.text = widget.data['flat_name'] ?? '';
    ...
    controller.UnitController.text =
        widget.data['unit'] ?? ''; // Note: Overwrites earlier UnitController
    ...
    controller.MinimumPreferredController.text =
    ...
    controller.selectedConstructionStatus.value = widget.data['construction_status'] ?? '';
    controller.aboutController.text =
        widget.data['project_description']??'';
  }
  void setPages() {
    ...
    if (index == 4 && controller.selectedIndex.value == 1) {
      if (controller.selectedIndex.value == 0) {
        ...
        if (controller.selectedIndex.value == 1 &&
            (controller.getProjectPropertyList.isEmpty && controller.getProjectPropertyList.isEmpty)  ) {
          ContinueWithAIOrMannual(context);
        } else {

          if(controller.getProjectPropertyList.isEmpty && controller.getProjectPropertyList.isEmpty ){
            Get.snackbar(
              'Missing Project',
  ...
  void goToPreviousView() {
    setState(() {
      if (widget.isfrom == 'property') {
        ...
        if (controller.currentViewIndex.value == 1) {
  ...
  void ClearFiled() {
    ...
    controller.getProjectPropertyList.clear();
  ...
  //   // }
  //
  //   final isListEmpty = controller.getProjectPropertyList.isEmpty && controller.getProjectPropertyList.isEmpty;
  //
  ...
  //
  //     controller.showprojectError.value = controller.getProjectPropertyList.isEmpty && controller.getProjectPropertyList.isEmpty;
  ...
  //       );
  //     }
  //     // if (controller.getProjectPropertyList.isEmpty) {
  //     //   Fluttertoast.showToast(
  ...
  //
  //     print("App project");
  //     print(controller.getProjectPropertyList.isEmpty);
  //   } else {
  ...
  bool validationProjectAndArea() {
    print("validationProjectAndArea==>");
    bool isValid = true;

    if (controller.editMode.value || controller.addMode.value) {
      bool hasUnsavedChanges = controller.bhkType.value.isNotEmpty ||
          controller.buildUpAreaController.text.isNotEmpty ||
          controller.selectedsqfit.value.isNotEmpty ||
          controller.selectedarea.value.isNotEmpty ||
          controller.propertyPriceController.text.isNotEmpty ||
          controller.projectPropertyImage.isNotEmpty;

      if (hasUnsavedChanges) {
        Fluttertoast.showToast(
          msg: 'Please Save the Project to proceed further',
        );
        return false;
      }
    }
    ...
    if (controller.getProjectPropertyList.isEmpty) {
      ...
      controller.showprojectError.value = controller.getProjectPropertyList.isEmpty;
      if (controller.showprojectError.value) isValid = false;
      ...
      // If getProjectPropertyList is not empty, clear error flags
      controller.showbhkError.value = false;
      controller.showimageError.value = false;
  ...
  bool validationAddImages() {
    ...
    print("controller.coverImgs.value == null || controller.coverImage.value==""===");
    print(controller.coverImgs.value == null && controller.coverImage.value=="");
    if (controller.coverImgs.value == null && controller.coverImage.value=="") {
      controller.showimageError.value = true;
      return false;
    }
    if ( controller.logoImgs.value == null  && controller.logoString.value=="") {
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Obx(() => controller.locationError.value
                      ...
                      child: Text(
                        'Please select all location fields (Country, State, City)',
                        style: TextStyle(
                          color: Colors.red,
                          fontSize: 12,
                        ),
        ...
        children: [
          ...
          Expanded(
              child: Obx(() => ListView.builder(
                itemCount: controller.residentalpropertyTypes.length,
                itemBuilder: (context, index) {
                    ...
                    child: GestureDetector(
                      onTap: () {
                        setState(() {
                          ...
                          String selectedType =controller.residentalpropertyTypes[index];
        ...
        child: Column(
          ...
          children: [
            ...
            CustomTextFormField(
              ...
              labelText: 'Enter Amount',isRequired: true,
              validator: (value) {

                if (value == null || value.isEmpty) {
                  return controller.categoryPriceselected.value == 0 ? 'Please Enter Property Price' : 'Please Enter Project Price';
                }
                return null;
              },
            ),
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  if (controller.selectedPropertyType.value ==
                      "Apartment")
                    ...getApartmentProjectWidgets(controller, context),
                  if (controller.selectedPropertyType.value == "Plot")
                    ...getPlotProjectWidgets(controller, context),
                  if (controller.selectedPropertyType.value ==
                      "Independent House")
                    ...getIndependentProjectHouseWidgets(
                        controller, context),
                  if (controller.selectedPropertyType.value ==
                      "Villa")
                    ...getVillaProjectsWidgets(controller, context),
                  if (controller.selectedPropertyType.value ==
                      "Builder Floor")
                    ...getBuilderProjectFloorWidgets(controller, context),
                  if (controller.selectedPropertyType.value ==
                      "Penthouse")
                    ...getPenthouseProjectWidgets(controller, context),
                  if (controller.selectedPropertyType.value == "PG")
                    ...getPGWidgets(controller, context),
                ],
              ),
...
class _ProjectPriceDetailsState extends State<projectPriceDetails> {
  ...
  void clearForm() {
    ...
    if(controller.getProjectPropertyList.isEmpty){
      setState(() {
        controller.addMode.value=true;
      });
    }else{
      controller.addMode.value = false;
    }
  }
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            controller.getProjectPropertyList.isNotEmpty?
                          ...
                          child: OutlinedButton(
                            onPressed: () {
                              ...
                              // );
                               ...
                               controller.postProjectProperty(
                              project_id: widget.id,
                              price: controller.propertyPriceController.text,
                              bhk: controller.bhkType.value,
                              area:controller.buildUpAreaController.text,
                              area_in:controller.selectedsqfit.value,
                              area_type:controller.selectedarea.value,
                              isfrom: 'edit',
                              xFile: XFile(controller.projectPropertyImage[0].path),
                              );

                              // .then((value) async {
                              // await controller.getPropertyProject(widget.id);
                              // });
                              setState(() {
                                // if (editingIndex != null) {
                                //   controller.getProjectPropertyList[editingIndex!] = detail;
                                //   editingIndex = null;
                                // } else {
                                //   controller.getProjectPropertyList.add(detail);
                                // }
                                clearForm();
                            ...
                            child: Text(
                              editingIndex != null ? 'Update Entry' : 'Save Project',
                              style: TextStyle(
                    ...
                    controller.getProjectPropertyList.isNotEmpty
                        ? Column(
                          ...
                          //   shrinkWrap: true,
                          //   physics: const NeverScrollableScrollPhysics(),
                          //   itemCount: controller.getProjectPropertyList.length,
                          //   itemBuilder: (context, index) {
                          //     final item = controller.getProjectPropertyList[index];
                          ...
                          //                     onPressed: () {
                          //                       setState(() {
                          //                         controller.getProjectPropertyList.removeAt(index);
                          //
                          //                         clearForm();
                          //                         editingIndex = null;
                          //                         calculateAveragePrice();
                                    ...
                                    children: [
                                      if (item['image'] != null)
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            borderRadius: BorderRadius.circular(8),
                                            image: DecorationImage(
                                              image: NetworkImage("http://13.127.244.70:4444/"+item['image'] ), // 👈 Pass your image URL here
                                              fit: BoxFit.cover,
                                            ),
                                          ),
                                        )
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            Text(
                                              '${item['bhk'] .isNotEmpty ? item['bhk'] : "N/A"}  ${controller.formatIndianPrice(item['price'].toString())}',
                                              style: const TextStyle(fontWeight: FontWeight.bold),
                                            ),
                                            const SizedBox(height: 4),
                                            Text(
                                              '${item['area']} ${item['area_in']}, ${item['area_type']}',
                                              style: TextStyle(color: Colors.grey[700]),
                                            ),
                                          ],
                                        ),
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            TextButton(
                                              onPressed: () {
                                                controller.removeProjectImages(
                                                  id: controller.getProjectPropertyList[index]['_id'],
                                                );
                                                setState(() {
                          ...
                          const SizedBox(height: 20),
                          controller.getProjectPropertyList.isEmpty|| controller.addMode.value?
                          const SizedBox():
                          SizedBox(
                            width: Get.width,
                            child: OutlinedButton(
                              ...
                              child: Text(
                                controller.getProjectPropertyList.isNotEmpty? 'Add More Project':'Add Project',
                                style: TextStyle(
                                  color: Colors.purple.withOpacity(0.8),
                                  fontSize: 16,
                                  fontWeight: FontWeight.w600,
  ...
  void calculateAveragePrice() {
    if (controller.getProjectPropertyList.isEmpty) {
      controller.avgProjectPriceController.text = '';
      controller.configurationController.text = '';
      return;
    }

    // Extract prices as doubles to handle floating-point values
    List<double> prices = controller.getProjectPropertyList
    ...
    if (prices.isEmpty) {
      ...
      // Format prices as integers (since you want whole numbers like 45000)
      String formattedMin = min.toInt().toString();
      String formattedMax = max.toInt().toString();

      // Set the price range (e.g., "45000 - 65000")
      controller.avgProjectPriceController.text =
      min == max ? formattedMin : '$formattedMin - $formattedMax';
    }

    // Extract unique BHK types
    final bhkTypes = controller.getProjectPropertyList
        .map((e) => e['bhk'].toString().trim())
        .where((bhk) => bhk.isNotEmpty)
        .toSet()
        .toList();


    controller.configurationController.text = bhkTypes.join(', ');
    print("controller.avgProjectPriceController.text => ${controller.avgProjectPriceController.text}");
    print("controller.configurationController.text => ${controller.configurationController.text}");
  }


  String formatShortIndianPrice(int amount) {
    if (amount >= 10000000) {
